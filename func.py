...
# keystroke Identification and normalization
...
def get_steam_library(api_key: str | None, steam_id: str | None, base_url: str = "http://api.steampowered.com") -> dict[int, str]:
    ...
    Returns:
        dict[int, str]: A dictionary of {appid: game_name} for the user's library.
...
async def filter_games_by_tier(games: dict[int, str], tierFilter: str) -> list[tuple[str, str, str]]:
    """
    Filters a game library by a specified tier.

    Parameters:
        games ( dict [int, str] ): The dictionary of games to filter by ID.
        tierFilter (str): The tier to filter by.

    Returns:
        list[tuple[str, str, str]]: only games of the sepecified tier
    """
    filtered_games = []
    for i, (appid, name) in enumerate(games.items(), 1):
        tier = await check_proton_compatibility(str(appid))
        if tierFilter == "Incompatible":
            if (tier == "borked"):
                filtered_games.append((name, str(appid), "Borked(Incompatible)"))
            else:
                pass
        elif tierFilter == "Bronze":
            if (tier == "bronze"):
                filtered_games.append((name, str(appid), "Bronze"))
            else:
                pass
        elif tierFilter == "Silver":
            if (tier == "silver"):
                filtered_games.append((name, str(appid), "Silver"))
            else:
                pass
        elif tierFilter == "Gold":
            if (tier == "gold"):
                filtered_games.append((name, str(appid), "Gold"))
            else:
                pass
        elif tierFilter == "Platinum":
            if (tier == "platinum"):
                filtered_games.append((name, str(appid), "Platinum"))
            else:
                pass
        # compatible mode
        else:
            if (tier == "borked"):
                ...
                filtered_games.append((name, str(appid), tier))
